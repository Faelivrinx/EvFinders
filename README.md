...
1. Firstly we need to expose to our application, server which can handle requests.
   - Fortunetly it's really simple, we just need to clone some other repository [Server Repo](https://github.com/Faelivrinx/evfinder-backend).
   - Now in cloned project directory, we can fire `./mvnw clean package` command that will create jar file.
   - Last step is to execute jar file. To do that we will use `java -jar <path-to-jar-file>`
